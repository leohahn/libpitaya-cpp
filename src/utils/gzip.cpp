...
int
Decompress(uint8_t** output, size_t* output_size, uint8_t* data, size_t size)
{
    z_stream inflate_s;
    ...
    // values of this parameter result in better compression at the expense of memory usage. This
    // range of values also changes the decoding type:
    ...
    *output = NULL;
    ...
    do {
        ...
        {
            unsigned char* new_output = (unsigned char*)realloc(*output, resize_to);
            if (!new_output) {
        ...
        if (ret != Z_STREAM_END && ret != Z_OK && ret != Z_BUF_ERROR) {
            ...
            LOG(Error) << "error decompressing: " << error_msg;
...
int
Compress(uint8_t** output, size_t* output_size, uint8_t* data, size_t size)
{
    int ret;
    z_stream strm;
    ...
    *output_size = CHUNK;
    ...
    strm.next_in = data;
    strm.next_out = *output = (unsigned char*)realloc(*output, *output_size);
...
int
IsCompressed(const uint8_t* data, size_t size)
{
    return size > 2 && (
                           // zlib
                           (data[0] == 0x78 && (data[1] == 0x9C || data[1] == 0x01 ||
